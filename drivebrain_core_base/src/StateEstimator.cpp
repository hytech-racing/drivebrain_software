...
std::pair<core::VehicleState, bool> StateEstimator::get_latest_state_and_validity()
{
    auto state_is_valid = _validate_stamps(_timestamp_array);
    {
        ...
        current_inputs->set_accel_percent(_vehicle_state.input.requested_accel);
        current_inputs->set_brake_percent(_vehicle_state.input.requested_brake);

        hytech_msgs::xyz_vector *current_body_vel_ms = msg_out->mutable_current_body_vel_ms();
        current_body_vel_ms->set_x(_vehicle_state.current_body_vel_ms.x);
        current_body_vel_ms->set_y(_vehicle_state.current_body_vel_ms.y);
        current_body_vel_ms->set_z(_vehicle_state.current_body_vel_ms.z);

        hytech_msgs::xyz_vector *current_body_accel_mss = msg_out->mutable_current_body_accel_mss();
        current_body_accel_mss->set_x(_vehicle_state.current_body_accel_mss.x);
        current_body_accel_mss->set_y(_vehicle_state.current_body_accel_mss.y);
        current_body_accel_mss->set_z(_vehicle_state.current_body_accel_mss.z);

        hytech_msgs::xyz_vector *current_angular_rate_rads = msg_out->mutable_current_angular_rate_rads();
        current_angular_rate_rads->set_x(_vehicle_state.current_angular_rate_rads.x);
        current_angular_rate_rads->set_y(_vehicle_state.current_angular_rate_rads.y);
        current_angular_rate_rads->set_z(_vehicle_state.current_angular_rate_rads.z);

        hytech_msgs::ypr_vector *current_ypr_rad = msg_out->mutable_current_ypr_rad();
        current_ypr_rad->set_yaw(_vehicle_state.current_ypr_rad.yaw);
        current_ypr_rad->set_pitch(_vehicle_state.current_ypr_rad.pitch);
        current_ypr_rad->set_roll(_vehicle_state.current_ypr_rad.roll);

        hytech_msgs::veh_vec_float *curr_rpms = msg_out->mutable_current_rpms();
        curr_rpms->set_fl(_vehicle_state.current_rpms.FL);
        curr_rpms->set_fr(_vehicle_state.current_rpms.FR);
        curr_rpms->set_rl(_vehicle_state.current_rpms.RL);
        curr_rpms->set_rr(_vehicle_state.current_rpms.RR);
