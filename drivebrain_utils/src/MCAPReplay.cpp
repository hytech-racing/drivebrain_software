...
MCAPReplay::MCAPReplay(std::string param_file_path, std::string dbc_file_path)
    ...
    _vcr_sender = std::make_shared<comms::ETHSendComms>(nullptr, _io_context, 9999, "127.0.0.1", false);
...
void MCAPReplay::start(std::string filename) {
    ...
    for (auto it = msg_view.begin(); it != msg_view.end(); it++) {
        ...
        } else if(it->schema->name == "hytech_msgs.VCRData_s")
        {
            _vcr_sender->enqueue_msg_to_send(msg);
        }
