...
core::FoxgloveWSServer::FoxgloveWSServer(
    std::vector<core::common::Configurable *> configurable_components,
    std::function<void(const std::string)> param_log_func)
    : _components(configurable_components), _log_param_change_func(param_log_func) {
    _log_handler = [](foxglove::WebSocketLogLevel, char const *msg) {
        std::cout << msg << std::endl;
    };

    _server_options.capabilities.push_back("parameters");
    _server_options.sendBufferLimitBytes = 100000000UL; 
    _server = foxglove::ServerFactory::createServer<websocketpp::connection_hdl>(
        "T.A.R.S", _log_handler, _server_options);

    foxglove::ServerHandlers<foxglove::ConnHandle> hdlrs;

    hdlrs.parameterRequestHandler = [this](const std::vector<std::string> &param_names,
    ...
    hdlrs.parameterChangeHandler = [&](const std::vector<foxglove::Parameter> &params,
                                       const std::optional<std::string> &request_id,
                                       foxglove::ConnHandle clientHandle) {
    ...
    for (const auto &file_descriptor : descriptors) {
        ...
        for (int i = 0; i < file_descriptor->message_type_count(); ++i) {
            ...
            server_channel.schema = foxglove::base64Encode(
...
void core::FoxgloveWSServer::send_live_telem_msg(std::shared_ptr<google::protobuf::Message> msg) {
    ...
    if (_id_name_map.find(msg->GetDescriptor()->name()) != _id_name_map.end()) {
        ...
        _server->broadcastMessage(msg_chan_id, now,
                                  reinterpret_cast<const uint8_t *>(serializedMsg.data()),
                                  serializedMsg.size());
    }
}

foxglove::Parameter
core::FoxgloveWSServer::_get_foxglove_param(const std::string &set_name,
                                            core::common::Configurable::ParamTypes param) {
    if (std::holds_alternative<bool>(param)) {
        ...
        // std::cout << set_name << " Variant holds a float: " << std::get<float>(param) <<
        // std::endl;
        ...
        // std::cout << set_name << " Variant holds a string: " << std::get<std::string>(param) <<
        // std::endl;
...
core::common::Configurable::ParamTypes
core::FoxgloveWSServer::_get_db_param(foxglove::Parameter param) {
...
std::optional<foxglove::Parameter> core::FoxgloveWSServer::_convert_foxglove_param(
    core::common::Configurable::ParamTypes curr_param_val, foxglove::Parameter incoming_param) {
    foxglove::ParameterType current_param_type =
        _get_foxglove_param(incoming_param.getName(), curr_param_val).getValue().getType();
    foxglove::ParameterType type = incoming_param.getValue().getType();

    using fpt = foxglove::ParameterType;
    if (_get_foxglove_param(incoming_param.getName(), curr_param_val).getValue().getType() ==
        incoming_param.getValue().getType()) {
...
void core::FoxgloveWSServer::_set_db_param(foxglove::Parameter param_update) {
    ...
    for (const auto component : _components) {
        if (component->get_name() == component_name) {
            ...
            core::common::Configurable::ParamTypes curr_param_val =
                component->get_cached_param(param_name);
            std::optional<foxglove::Parameter> converted_type =
                _convert_foxglove_param(curr_param_val, param_update);
            if (converted_type) {
...
std::vector<foxglove::Parameter> core::FoxgloveWSServer::_get_current_params() {
    ...
    for (const auto component : _components) {
        ...
        for (const auto &component_param_name : param_names) {
            ...
            foxglove::Parameter fxglove_param =
                _get_foxglove_param(foxglove_param_id, params_map[component_param_name]);
